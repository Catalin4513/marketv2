...
RUN apt-get update && apt-get install -y \
  ...
  docker-php-ext-configure gd --enable-gd --with-freetype=/usr/local/lib --with-jpeg=/usr/local/lib && \
docker-php-ext-install zip pdo_mysql mysqli intl gd soap curl opcache posix simplexml tidy exif

ENV APACHE_DOCUMENT_ROOT /var/www/public

RUN sed -ri -e 's!/var/www/html!${APACHE_DOCUMENT_ROOT}!g' /etc/apache2/sites-available/*.conf && \
sed -ri -e 's!/var/www/!${APACHE_DOCUMENT_ROOT}!g' /etc/apache2/apache2.conf /etc/apache2/conf-available/*.conf && \
a2enmod rewrite proxy_http ssl headers && \
...
curl -sL https://deb.nodesource.com/setup_12.x -o nodesource_setup.sh && bash nodesource_setup.sh && apt-get -y --force-yes install nodejs
